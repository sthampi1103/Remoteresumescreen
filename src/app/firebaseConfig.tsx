            ...
            if (appCheckError instanceof Error) {
                if ((appCheckError as any).code === 'appCheck/fetch-status-error') {
                     console.error(
                        ...
                        "VERIFY IMMEDIATELY:\n" +
                        "1. DOMAIN AUTHORIZATION (Google Cloud Console): Is your app's domain (e.g., 'localhost', 'your-project-id.web.app', 'region.hosted.app') *EXPLICITLY* listed as an authorized domain for your reCAPTCHA Enterprise key? Go to Google Cloud Console -> Security -> reCAPTCHA Enterprise -> YOUR KEY -> Settings -> Domains. THIS IS THE MOST COMMON CAUSE. Try adding 'http://localhost' and 'https://localhost' if using localhost.\n" +
                    ...
                    console.error(
                        ...
                        "1. DOMAIN AUTHORIZATION (Google Cloud): Is your app's domain (e.g., 'localhost', 'your-deployed-url.web.app', 'region.hosted.app') *EXPLICITLY* listed as an authorized domain in Google Cloud Console for this reCAPTCHA Enterprise key? Try adding 'http://localhost' and 'https://localhost' if using localhost.\n" +
...
if (appInitialized && authInitialized && !appCheckInitialized && process.env.NEXT_PUBLIC_FIREBASE_RECAPTCHA_ENTERPRISE_SITE_KEY) {
    console.error(
        ...
        "  1. **DOMAIN NOT AUTHORIZED (Google Cloud Console):** Your app's domain (e.g., 'localhost', 'your-project.web.app', 'region.hosted.app') MUST be in the allowed list for your reCAPTCHA Enterprise key. Try adding 'http://localhost' AND 'https://localhost' if developing locally.\n" +
